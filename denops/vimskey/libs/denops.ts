...
export const saveInput = async (denops: Denops, params: {
  ...
  const vimValue = await getVimValue(denops, { type: params.type, name: params.type })
  ...
  if (userInput) {
    await setVimValue(denops, { type: params.type, name: params.name, value: userInput })
    return userInput
  } else {
...
export const setVimValue = async (denops: Denops, params: {
  ...
  await variable[params.type].set(denops, `vimskey#${params.name}`, params.value)
}
